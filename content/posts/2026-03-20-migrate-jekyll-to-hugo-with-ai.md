...
Jekyll 을 이용해서 블로그를 시작한 게 2018년이니 어언 8년이 됐다. 
그동안 Jekyll 기반으로 GitHub Pages 에 올려서 운영해왔는데, 솔직히 말하면 루비(Ruby) 환경 셋업이 귀찮을 때마다 포스팅 의욕이 뚝 떨어졌다. `bundle install` 이 실패하면 반나절이 날아가는 경험을 몇 번 하고 나니 Hugo 로 옮겨야겠다는 생각이 굳어졌다.

마침 Claude Code 를 쓰기 시작하면서 "이거 한번 시켜볼까?" 싶었다. 결론부터 말하면, "**손 안 대고 코 풀었다.**" 라고 할만큼 나는 한 거 없이, 너무 쉽게 처리됐다.
